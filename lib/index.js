...
function createFeishuRpcHandler(controller, { encodeQr = qrCodeDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    const payloadFailure3 = validPayload(endpoint, payload);
    if (payloadFailure3) return badRequest2(payloadFailure3);
    try {
      let value;
      if (endpoint === FEISHU_ENDPOINTS.status) {
...
import { randomBytes, randomUUID as randomUUID8 } from "node:crypto";
      ...
      const response = await requestJson2(fetchImpl, {
        ...
        body: {
          msg: {
            ...
            client_id: `dsh-weixin-${randomUUID8()}`,
            message_type: 2,
            message_state: 2,
...
// src/channels/weixin/config-store.mjs
var EMPTY_DOCUMENT2 = Object.freeze({ version: 1, accounts: Object.freeze([]) });
...
function deriveWeixinBotIdentity(accountId) {
  const raw = cleanString5(accountId);
...
function maskWeixinAccountId(accountId) {
  const value = cleanString5(accountId) ?? "";
  if (value.length <= 10) return value ? `${value.slice(0, 3)}\u2022\u2022\u2022` : "\u5FAE\u4FE1\u673A\u5668\u4EBA";
  return `${value.slice(0, 6)}\u2022\u2022\u2022\u2022${value.slice(-4)}`;
}
function normalizeAccount(value) {
  if (!value || typeof value !== "object") return null;
  const accountId = cleanString5(value.accountId);
  const ownerUserId = cleanString5(value.ownerUserId);
...
var WeixinConfigStore = class {
  ...
  async remove(botId) {
    if (!safeBotId2(botId)) throw new TypeError("Invalid Weixin bot id");
    return this.#mutate((accounts) => {
...
var HarnessClient3 = class {
  ...
  async rpc(method, payload = {}, timeoutMs = 3e4, options = {}) {
    const rpcId = options.rpcId ?? `weixin-${randomUUID9()}`;
    const response = await fetch(new URL(`/api/${method}`, this.#baseUrl), {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ type: "client-request", rpcId, method, payload }),
  ...
  async ask(sessionId, text, options = {}) {
    ...
    const before = await this.rpc("session.history", { sessionId, maxMessages: 1 });
    const baselineSeq = Math.max(-1, ...(before.events ?? []).map(({ event }) => event.seq ?? -1));
    const promptRpcId = `weixin-${randomUUID9()}`;
...
var EMPTY_STATE3 = Object.freeze({
  version: 1,
  sessions: {},
  seenMessageIds: [],
  getUpdatesBuf: ""
...
var ACTIVE_ATTEMPT_STATES2 = /* @__PURE__ */ new Set([
  "starting",
  "pending",
  "scanned",
  "needs_verification",
...
var TERMINAL_ATTEMPT_STATES2 = /* @__PURE__ */ new Set(["connected", "expired", "failed", "cancelled"]);
var QR_TTL_MS = 5 * 6e4;
...
var WeixinController = class {
  ...
  async submitVerification(attemptId, verifyCode) {
    ...
    const code = cleanString6(verifyCode);
  ...
  status() {
    ...
    return {
      schemaVersion: 1,
      revision: this.#revision,
      state: active && ACTIVE_ATTEMPT_STATES2.has(active.state) ? "provisioning" : accounts.length === 0 ? "disconnected" : connectedCount === accounts.length ? "connected" : connectedCount > 0 ? "degraded" : "offline",
      bots: accounts,
  ...
  async #runProvisioning(record) {
    try {
      while (!record.controller.signal.aborted && Date.now() < record.expiresAt) {
        if (record.state === "needs_verification" && !record.pendingVerifyCode) {
          await new Promise((resolve4) => {
            record.verifyResolve = resolve4;
            if (record.controller.signal.aborted) resolve4();
          });
          record.verifyResolve = null;
          this.#assertAttemptActive(record);
        }
        ...
        if (response.status === "wait") {
          ...
          const token = cleanString6(response.bot_token);
          const accountId = cleanString6(response.ilink_bot_id);
          const ownerUserId = cleanString6(response.ilink_user_id);
      ...
      if (!record.controller.signal.aborted && Date.now() >= record.expiresAt && !TERMINAL_ATTEMPT_STATES2.has(record.state)) {
        record.state = "expired";
        record.error = safeAccountError("expired", "\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F\uFF0C\u8BF7\u91CD\u65B0\u751F\u6210\u3002");
      }
    } catch (error) {
  ...
  #pruneAttempts() {
    for (const [id, record] of this.#attempts) {
      if (id !== this.#activeAttemptId && TERMINAL_ATTEMPT_STATES2.has(record.state) && this.#attempts.size > 16) {
        this.#attempts.delete(id);
      }
    }
  }
...
function conversationKey3(userId) {
  return `p2p:${userId}`;
}
...
var WeixinHarnessBridge = class {
  ...
  async #process(message) {
    ...
    try {
      ...
      if (command === "/help") {
        await this.#send(sender, HELP_TEXT3, contextToken, runId);
        await this.#state.markSeen(messageId);
        return;
      }
      if (command === "/status") {
      ...
      if (command === "/new") {
        await this.#state.clearSession(conversationKey3(sender));
        await this.#send(sender, "\u5DF2\u5F00\u542F\u65B0\u4F1A\u8BDD\u3002\u8BF7\u53D1\u9001\u4F60\u7684\u95EE\u9898\u3002", contextToken, runId);
        await this.#state.markSeen(messageId);
        return;
      }
      const key = conversationKey3(sender);
      let sessionId = this.#state.sessionFor(key);
      if (!sessionId || !await this.#harness.sessionExists(sessionId)) {
        sessionId = await this.#harness.createSession();
        await this.#state.setSession(key, sessionId);
...
function pluginPaths3(config) {
  ...
  const root = resolve3(config.dataDir ?? join3(dshHome, "integrations", "dsh-weixin"));
  return {
    root,
    config: resolve3(config.configPath ?? join3(root, "config.json")),
    accounts: resolve3(config.accountsDir ?? join3(root, "accounts"))
  };
}
async function createProductionController3(ctx, config = {}, internals = {}) {
  ...
  const configStore = await new ConfigStore(paths.config).load();
  const stateStores = /* @__PURE__ */ new Map();
  const statePath = (botId) => resolve3(paths.accounts, botId, "state.json");
  const stateFor = async (botId) => {
    let state = stateStores.get(botId);
    if (!state) {
      state = await new StateStore2(statePath(botId)).load();
...
function payloadFailure2(endpoint, payload) {
  ...
  if (endpoint === WEIXIN_ENDPOINTS.submitVerification) {
    return exactKeys2(payload, ["attemptId", "verifyCode"]) && validId2(payload.attemptId) && typeof payload.verifyCode === "string" && /^\d{4,8}$/.test(payload.verifyCode) ? null : "provision.verify requires an attemptId and a 4-to-8-digit code.";
  }
...
async function publicStatus2(status, encodeQr) {
  ...
  if (safe.provisioning) safe.provisioning = await withEncodedQr2(safe.provisioning, encodeQr);
  return safe;
}
...
function createWeixinRpcHandler(controller, { encodeQr = qrDataUrl2 } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      let value;
      if (endpoint === WEIXIN_ENDPOINTS.status) {
        value = await publicStatus2(await controller.status(), cachedEncode);
        ...
        value = await withEncodedQr2(started, cachedEncode);
...
function createImHostPlugin(internals = {}) {
  ...
  const startWeixin = internals.applyWeixin ?? apply3;
