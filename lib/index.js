...
function createFeishuRpcHandler(controller, { encodeQr = qrCodeDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    const payloadFailure4 = validPayload(endpoint, payload);
    if (payloadFailure4) return badRequest2(payloadFailure4);
    try {
      let value;
      if (endpoint === FEISHU_ENDPOINTS.status) {
...
var QqController = class {
  ...
  async startProvisioning() {
    ...
    let firstQrResolve;
    let firstQrReject;
    const firstQr = new Promise((resolve5, reject) => {
      firstQrResolve = resolve5;
...
import { randomBytes, randomUUID as randomUUID10 } from "node:crypto";
...
async function requestJson2(fetchImpl, {
  method,
  baseUrl,
  endpoint,
  body,
...
function createWeixinApi({ fetchImpl = fetch } = {}) {
  ...
  return Object.freeze({
    async beginLogin({ localTokens = [], botType = DEFAULT_BOT_TYPE, signal } = {}) {
      ...
      const response = await requestJson2(fetchImpl, {
        method: "POST",
    ...
    async getUpdates({ baseUrl, token, getUpdatesBuf = "", timeoutMs, signal }) {
      try {
        return await requestJson2(fetchImpl, {
          method: "POST",
          baseUrl,
      ...
      const response = await requestJson2(fetchImpl, {
        ...
        body: {
          msg: {
            ...
            client_id: `dsh-weixin-${randomUUID10()}`,
            message_type: 2,
            message_state: 2,
...
// src/channels/weixin/config-store.mjs
var EMPTY_DOCUMENT3 = Object.freeze({ version: 1, accounts: Object.freeze([]) });
...
function deriveWeixinBotIdentity(accountId) {
  const raw = cleanString7(accountId);
...
function maskWeixinAccountId(accountId) {
  const value = cleanString7(accountId) ?? "";
  if (value.length <= 10) return value ? `${value.slice(0, 3)}\u2022\u2022\u2022` : "\u5FAE\u4FE1\u673A\u5668\u4EBA";
  return `${value.slice(0, 6)}\u2022\u2022\u2022\u2022${value.slice(-4)}`;
}
function normalizeAccount(value) {
  if (!value || typeof value !== "object") return null;
  const accountId = cleanString7(value.accountId);
  const ownerUserId = cleanString7(value.ownerUserId);
...
var EMPTY_STATE4 = Object.freeze({
  version: 1,
  sessions: {},
  seenMessageIds: [],
  getUpdatesBuf: ""
...
var ACTIVE_ATTEMPT_STATES3 = /* @__PURE__ */ new Set([
  "starting",
  "pending",
  "scanned",
  "needs_verification",
...
var TERMINAL_ATTEMPT_STATES3 = /* @__PURE__ */ new Set(["connected", "expired", "failed", "cancelled"]);
...
var WeixinController = class {
  ...
  async startProvisioning() {
    ...
    try {
      ...
      record.state = "pending";
      record.expiresAt = Date.now() + QR_TTL_MS2;
      this.#touch();
  ...
  registrationStatus(attemptId) {
    return publicAttempt3(this.#attempts.get(attemptId));
  }
  async submitVerification(attemptId, verifyCode) {
    ...
    const code = cleanString8(verifyCode);
  ...
  status() {
    ...
    return {
      schemaVersion: 1,
      revision: this.#revision,
      state: active && ACTIVE_ATTEMPT_STATES3.has(active.state) ? "provisioning" : accounts.length === 0 ? "disconnected" : connectedCount === accounts.length ? "connected" : connectedCount > 0 ? "degraded" : "offline",
      bots: accounts,
  ...
  async #runProvisioning(record) {
    try {
      while (!record.controller.signal.aborted && Date.now() < record.expiresAt) {
        if (record.state === "needs_verification" && !record.pendingVerifyCode) {
          await new Promise((resolve5) => {
            record.verifyResolve = resolve5;
            if (record.controller.signal.aborted) resolve5();
          });
          record.verifyResolve = null;
          this.#assertAttemptActive(record);
        }
        ...
        if (response.status === "wait") {
          ...
          const token = cleanString8(response.bot_token);
          const accountId = cleanString8(response.ilink_bot_id);
          const ownerUserId = cleanString8(response.ilink_user_id);
      ...
      if (!record.controller.signal.aborted && Date.now() >= record.expiresAt && !TERMINAL_ATTEMPT_STATES3.has(record.state)) {
        record.state = "expired";
        record.error = safeAccountError("expired", "\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F\uFF0C\u8BF7\u91CD\u65B0\u751F\u6210\u3002");
      }
    } catch (error) {
  ...
  async #resolveToken(ref) {
    const result = await this.#credentials.resolve(ref).catch(() => void 0);
    return cleanString8(result?.value);
  }
  async #restoreCredential(ref, previous) {
    try {
  ...
  #pruneAttempts() {
    for (const [id, record] of this.#attempts) {
      if (id !== this.#activeAttemptId && TERMINAL_ATTEMPT_STATES3.has(record.state) && this.#attempts.size > 16) {
        this.#attempts.delete(id);
      }
    }
  }
...
function conversationKey4(userId) {
  return `p2p:${userId}`;
}
...
var WeixinHarnessBridge = class {
  ...
  async #process(message) {
    ...
    try {
      ...
      if (command === "/help") {
        await this.#send(sender, HELP_TEXT4, contextToken, runId);
        await this.#state.markSeen(messageId);
        return;
      }
      if (command === "/status") {
      ...
      if (command === "/new") {
        await this.#state.clearSession(conversationKey4(sender));
        await this.#send(sender, "\u5DF2\u5F00\u542F\u65B0\u4F1A\u8BDD\u3002\u8BF7\u53D1\u9001\u4F60\u7684\u95EE\u9898\u3002", contextToken, runId);
        await this.#state.markSeen(messageId);
        return;
      }
      const key = conversationKey4(sender);
      let sessionId = this.#state.sessionFor(key);
      if (!sessionId || !await this.#harness.sessionExists(sessionId)) {
        sessionId = await this.#harness.createSession();
        await this.#state.setSession(key, sessionId);
...
function pluginPaths4(config) {
  ...
  const root = resolve4(config.dataDir ?? join4(dshHome, "integrations", "dsh-weixin"));
  return {
    root,
    config: resolve4(config.configPath ?? join4(root, "config.json")),
    accounts: resolve4(config.accountsDir ?? join4(root, "accounts"))
  };
}
async function createProductionController4(ctx, config = {}, internals = {}) {
  ...
  const configStore = await new ConfigStore(paths.config).load();
  const stateStores = /* @__PURE__ */ new Map();
  const statePath = (botId) => resolve4(paths.accounts, botId, "state.json");
  const stateFor = async (botId) => {
    let state = stateStores.get(botId);
    if (!state) {
      state = await new StateStore2(statePath(botId)).load();
...
function payloadFailure3(endpoint, payload) {
  ...
  if (endpoint === WEIXIN_ENDPOINTS.status) {
    return exactKeys3(payload, []) ? null : "connection.status does not accept fields.";
  }
  ...
  if (endpoint === WEIXIN_ENDPOINTS.submitVerification) {
    return exactKeys3(payload, ["attemptId", "verifyCode"]) && validId3(payload.attemptId) && typeof payload.verifyCode === "string" && /^\d{4,8}$/.test(payload.verifyCode) ? null : "provision.verify requires an attemptId and a 4-to-8-digit code.";
  }
...
async function publicStatus3(status, encodeQr) {
  ...
  if (safe.provisioning) safe.provisioning = await withEncodedQr3(safe.provisioning, encodeQr);
  return safe;
}
...
function createWeixinRpcHandler(controller, { encodeQr = qrDataUrl3 } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      let value;
      if (endpoint === WEIXIN_ENDPOINTS.status) {
        value = await publicStatus3(await controller.status(), cachedEncode);
        ...
        value = await withEncodedQr3(started, cachedEncode);
...
function createImHostPlugin(internals = {}) {
  ...
  const startWeixin = internals.applyWeixin ?? apply4;
