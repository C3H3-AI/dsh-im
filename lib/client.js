...
function opaqueId(value) {
  const id4 = optionalString(value, 128);
  return id4 && /^[a-z\d_-]+$/i.test(id4) ? id4 : void 0;
}
...
function createTokenChannelApi(channel3, connectionSummary) {
  ...
  const normalizeSnapshot7 = (value) => {
  ...
  const presentError8 = (error) => ({
    code: text(error?.code, `${channel3.toUpperCase()}_ERROR`, 80),
    message: text(error?.message, `${channel3}\u64CD\u4F5C\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5`)
  });
  return Object.freeze({ unwrapRpcResult: unwrapRpcResult8, normalizeSnapshot: normalizeSnapshot7, presentError: presentError8 });
}
...
function createTokenChannelSettings(definition) {
  ...
  function AccountCard5({ account, busy, removing, onReconnect, onRequestRemove, onConfirmRemove, onCancelRemove }) {
    ...
    return h4(
      ...
      removing ? h4(
        ...
        h4("p", null, `\u8FD9\u4F1A\u505C\u6B62\u6D88\u606F\u8FDE\u63A5\uFF0C\u5E76\u5220\u9664\u672C\u673A\u4FDD\u5B58\u7684 Bot Token\u3001\u673A\u5668\u4EBA\u914D\u7F6E\u53CA\u4F1A\u8BDD\u6620\u5C04\u3002${platformLabel}\u4E2D\u7684\u673A\u5668\u4EBA\u4E0D\u4F1A\u88AB\u81EA\u52A8\u5220\u9664\u3002`),
        h4(
          "div",
          { className: "ddt-actions dim-viewActions" },
  ...
  function SettingsTab({ rpcCall }) {
    ...
    const bindCredentials = React4.useCallback(async ({ secret }) => {
      setBusy(true);
      setCredentialError(null);
      try {
        const snapshot = api3.normalizeSnapshot(await invoke(endpoints.bindCredentials, { token: secret }));
    ...
    return h4(
        ...
        h4(
          ...
          h4(
            ...
            h4(Button3, {
              ...
              disabled: busy,
              "aria-pressed": credentialOpen,
              "aria-label": `\u4F7F\u7528 Bot Token \u63A5\u5165 ${channel3} \u673A\u5668\u4EBA`
            }, h4(CredentialActionIcon), credentialOpen ? "\u6536\u8D77\u51ED\u636E" : "\u624B\u52A8\u63A5\u5165")
          ),
      ...
      model.phase === "loading" ? h4("div", {
        ...
        React4.Fragment,
        null,
        credentialOpen ? h4(CredentialBindingPanel, {
          channel: channel3,
...
function formatCheckedTime(timestamp6) {
  if (!timestamp6) return "\u5C1A\u672A\u68C0\u67E5";
  try {
    return new Intl.DateTimeFormat("zh-CN", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit"
    }).format(new Date(timestamp6));
  } catch {
    return "\u521A\u521A";
  }
}
...
function FeishuSettingsTab({ rpcCall }) {
    ...
    try {
      ...
      setModel((current) => ({
        ...
        provisioning: {
          phase: "qr",
          ...provision2,
          durationMs: Math.max(1, provision2.expiresAt - timestamp6),
          expired: false
        }
      }));
  ...
  React6.useEffect(() => {
    ...
    const tick = () => {
      const timestamp6 = Date.now();
      setNow(timestamp6);
      if (timestamp6 >= countdownExpiresAt) {
...
function checkedTime5(timestamp6) {
  if (!timestamp6) return "\u5C1A\u672A\u68C0\u67E5";
  try {
    return new Intl.DateTimeFormat("zh-CN", {
      hour: "2-digit",
...
function DingtalkLogo() {
  return h9(
    ...
    h9(DingtalkLogoGlyph)
  );
}
function QqLogo() {
  return h9("span", { className: "dim-logo dim-logoQq", "aria-hidden": "true" }, h9(QqLogoGlyph));
}
function WecomLogo() {
  return h9("span", { className: "dim-logo dim-logoWecom", "aria-hidden": "true" }, h9(WecomLogoGlyph));
}
function TelegramLogo() {
  return h9(
    "span",
    { className: "dim-logo dim-logoTelegram", "aria-hidden": "true" },
    h9(TelegramLogoGlyph)
  );
}
...
function ChannelLogo({ channel: channel3 }) {
  if (channel3 === "weixin") return h9(WeixinLogo);
  if (channel3 === "feishu") return h9(FeishuLogo);
  if (channel3 === "dingtalk") return h9(DingtalkLogo);
  if (channel3 === "wecom") return h9(WecomLogo);
  if (channel3 === "qq") return h9(QqLogo);
  if (channel3 === "telegram") return h9(TelegramLogo);
  return h9(DiscordLogo);
}
function IMSettingsTab({
  ...
  const [selected, setSelected] = React10.useState("weixin");
  ...
  return h9(
    ...
    h9(
      "header",
      { className: "dim-title" },
      h9("p", null, "\u628A\u673A\u5668\u4EBA\u63A5\u5165 DeepSeek Harness")
    ),
    h9(
      "div",
      ...
      h9(
        ...
        CHANNELS.map((channel3) => h9(
          ...
          h9(ChannelLogo, { channel: channel3.id }),
          h9(
            "span",
            { className: "dim-channelCopy" },
            h9("strong", null, channel3.label)
          )
        ))
      ),
      h9("div", { className: "dim-divider", "aria-hidden": "true" }),
      h9("main", {
        className: "dim-panel",
        role: "tabpanel",
        id: `dim-panel-${active.id}`,
        "aria-labelledby": `dim-tab-${active.id}`
      }, active.id === "weixin" ? h9(WeixinSettingsTab, { rpcCall: weixinRpcCall }) : active.id === "feishu" ? h9(FeishuSettingsTab, { rpcCall: feishuRpcCall }) : active.id === "dingtalk" ? h9(DingtalkSettingsTab, { rpcCall: dingtalkRpcCall }) : active.id === "wecom" ? h9(WecomSettingsTab, { rpcCall: wecomRpcCall }) : active.id === "qq" ? h9(QqSettingsTab, { rpcCall: qqRpcCall }) : active.id === "telegram" ? h9(TelegramSettingsTab, { rpcCall: telegramRpcCall }) : h9(DiscordSettingsTab, { rpcCall: discordRpcCall }))
    )
  );
}
function apply(ctx) {
