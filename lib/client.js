...
function validateContextEnhancementScope(input) {
  ...
  if (!Array.isArray(fields) || ![...fields].every((field2) => CONTEXT_ENHANCEMENT_FIELDS.includes(field2))) {
  ...
  return Object.freeze({
    enabled,
    fields: Object.freeze(CONTEXT_ENHANCEMENT_FIELDS.filter((field2) => fields.includes(field2))),
...
function ContextEnhancementScopeEditor({
  ...
  return h2(
    ...
    h2(
      ...
      h2("div", { className: "dim-contextFields" }, CONTEXT_ENHANCEMENT_FIELDS.map((field2) => {
        const fieldId = `${idPrefix}-${kind}-field-${field2}`;
        return h2(
          "div",
          { key: field2, className: "dim-contextField" },
          h2("input", {
            id: fieldId,
            type: "checkbox",
            name: `${kind}-${field2}`,
            checked: scope.fields.includes(field2),
            disabled,
            onChange: (event) => onChange("fields", event.target.checked ? [...scope.fields, field2] : scope.fields.filter((value) => value !== field2))
          }),
          h2(
            "span",
            { className: "dim-contextFieldText" },
            h2("label", { className: "dim-contextFieldName", htmlFor: fieldId }, FIELD_LABELS[field2]),
            FIELD_HELP[field2] ? h2(
              "span",
              ...
              h2("button", {
                ...
                disabled,
                "aria-label": copy[FIELD_HELP[field2].labelKey],
                "aria-describedby": `${idPrefix}-${kind}-${field2}-help`
              }, h2("span", { "aria-hidden": "true" }, "?")),
              h2("span", {
                id: `${idPrefix}-${kind}-${field2}-help`,
                className: "dim-contextTooltip dim-contextFieldTooltip",
                role: "tooltip"
              }, FIELD_HELP[field2].text)
            ) : null,
            h2("label", { className: "dim-contextFieldKey", htmlFor: fieldId }, field2)
          )
        );
      }))
    ),
...
function normalizeWeixinDiagnosticDetails(value) {
  ...
  const result = {};
  for (const [field2, allowed] of [["operation", OPERATIONS], ["stage", STAGES], ["reason", REASONS], ["resource", RESOURCES]]) {
    if (allowed.has(value[field2])) result[field2] = value[field2];
  }
...
function formatWeixinDiagnostic(value) {
  ...
  return [
    ...
    ...["operation", "stage", "reason", "httpStatus", "providerCode", "resource", "file", "field", "issue", "referenceId", "occurredAt", "rollback", "pluginVersion"].filter((field2) => details[field2] !== void 0).map((field2) => `${field2}: ${details[field2]}`)
  ].filter(Boolean).join("\n");
}
...
var EMAIL_RPC_CHANNEL = "/email";
var EMAIL_ENDPOINTS = Object.freeze({
  ...TOKEN_BOT_ENDPOINTS,
...
var api5 = createTokenChannelApi("Email", " IMAP/SMTP \u90AE\u7BB1");
...
// plugin-src/client/channels/email/styles.js
var EMAIL_STYLE_ID = "xmanrui-dsh-im-email-settings";
...
function installEmailStyles() {
  ...
  const style = document.createElement("style");
  style.dataset.plugin = "@xmanrui/dsh-im";
  style.dataset.pluginCss = EMAIL_STYLE_ID;
...
function field(label, control, hint) {
  return h2(
    "label",
    { className: "dim-emailField" },
    h2("span", null, label),
    control,
    hint ? h2("span", { className: "dim-emailHint" }, hint) : null
  );
}
function MailboxPanel({ busy, error, onSubmit, onCancel }) {
  const [provider, setProvider] = React26.useState("qq");
  const [address, setAddress] = React26.useState("");
  const [password, setPassword] = React26.useState("");
  const [allowedSenders, setAllowedSenders] = React26.useState("");
  const [imapHost, setImapHost] = React26.useState("");
  const [imapPort, setImapPort] = React26.useState("");
  const [smtpHost, setSmtpHost] = React26.useState("");
  const [smtpPort, setSmtpPort] = React26.useState("");
  ...
  const submit = () => onSubmit({
    ...
    password,
    provider,
    allowedSenders: allowedSenders.split(/[\s,;，；]+/).map((value) => value.trim()).filter(Boolean),
    ...custom ? {
      imapHost: imapHost.trim(),
      imapPort: imapPort.trim() || void 0,
      smtpHost: smtpHost.trim(),
      smtpPort: smtpPort.trim() || void 0
    } : {}
  });
  return h2(
    "section",
    { className: "ddt-card dim-surfaceCard dim-emailPanel" },
    h2("h3", null, "\u63A5\u5165\u90AE\u7BB1"),
    h2("p", null, "DeepSeek Harness \u4F1A\u8BFB\u53D6\u8BE5\u90AE\u7BB1\u7684\u65B0\u90AE\u4EF6\u4F5C\u4E3A\u6307\u4EE4\uFF0C\u5E76\u5728\u540C\u4E00\u90AE\u4EF6\u7EBF\u7A0B\u5185\u56DE\u590D\u5904\u7406\u7ED3\u679C\u3002"),
    h2(
      "div",
      { className: "dim-emailFields" },
      field(
        "\u90AE\u7BB1\u670D\u52A1\u5546",
        h2("select", {
          value: provider,
          ...
          disabled: busy
        }, PROVIDERS.map((entry) => h2("option", { key: entry.key, value: entry.key }, entry.label))),
        h2("span", { className: "dim-emailHint" }, PROVIDER_HINTS[provider])
      ),
      field("\u90AE\u7BB1\u5730\u5740", h2("input", {
        type: "email",
        value: address,
      ...
      custom ? h2(
        "div",
        { className: "dim-emailGrid" },
        field("IMAP \u670D\u52A1\u5668", h2("input", {
          value: imapHost,
...
var EMAIL_SETTINGS_DEFINITION = Object.freeze({
  channel: "Email",
...
var channel5 = createTokenChannelSettings(EMAIL_SETTINGS_DEFINITION);
var EmailSettingsTab = channel5.SettingsTab;
var EmailAccountCard = channel5.AccountCard;
...
function routeIdentity(definition, candidate) {
  ...
  for (const field2 of fields) {
    const value = candidate.route[field2.key];
...
function TargetForm({
  ...
  const currentTarget = () => {
    const normalizedRoute = Object.fromEntries(fieldsFor(definition, kind).map((field2) => {
      ...
      return [field2.key, field2.integer ? Number(raw) : raw];
    }));
    return {
  ...
  const testReady = fieldsFor(definition, kind).every((field2) => String(route[field2.key] ?? "").trim());
  return h2(
    "form",
    ...
    h2(
      ...
      fieldsFor(definition, kind).map((field2) => h2(
        "label",
        {
          key: field2.key,
          className: "dim-targetField"
        },
        h2("span", null, field2.label),
        h2("input", {
          name: field2.key,
          value: route[field2.key] ?? "",
          disabled: testing,
          required: true,
          inputMode: field2.inputMode,
          autoCapitalize: "none",
          autoCorrect: "off",
          spellCheck: false,
...
function suggestionOptionLabel(definition, suggestion, added) {
  ...
  const route = fieldsFor(definition, suggestion.kind).map((field2) => maskRouteValue(suggestion.route?.[field2.key])).filter(Boolean).join(" \xB7 ");
...
function DeliveryTargetSettingsPage({
  ...
  const selectSuggestion = (suggestion) => {
    const identity = routeIdentity(definition, suggestion);
    if (!identity) return;
    const route = Object.fromEntries(fieldsFor(definition, suggestion.kind).map((field2) => [
      field2.key,
      suggestion.route[field2.key]
    ]));
    setEditor({
      mode: "create",
      source: "suggestion",
...
function EmailLogo() {
  return h2(
    "span",
    { className: "dim-logo dim-logoEmail", "aria-hidden": "true" },
    h2(EmailLogoGlyph)
  );
}
function OfficeLogo() {
...
function ChannelLogo({ channel: channel6 }) {
  ...
  if (channel6 === "email") return h2(EmailLogo);
