...
function opaqueId(value) {
  const id2 = optionalString(value, 128);
  return id2 && /^[a-z\d_-]+$/i.test(id2) ? id2 : void 0;
}
...
function formatCheckedTime(timestamp4) {
  if (!timestamp4) return "\u5C1A\u672A\u68C0\u67E5";
  try {
    return new Intl.DateTimeFormat("zh-CN", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit"
    }).format(new Date(timestamp4));
  } catch {
    return "\u521A\u521A";
  }
}
...
function FeishuSettingsTab({ rpcCall }) {
    ...
    try {
      ...
      setModel((current) => ({
        ...
        provisioning: {
          phase: "qr",
          ...provision2,
          durationMs: Math.max(1, provision2.expiresAt - timestamp4),
          expired: false
        }
      }));
  ...
  React4.useEffect(() => {
    ...
    const tick = () => {
      const timestamp4 = Date.now();
      setNow(timestamp4);
      if (timestamp4 >= countdownExpiresAt) {
...
var PROVISION_STATES3 = /* @__PURE__ */ new Set([
  "starting",
  "pending",
  "scanned",
  "needs_verification",
...
function unwrapRpcResult4(result) {
  if (!isRecord4(result) || typeof result.ok !== "boolean") {
...
function normalizeProvisioning4(value) {
  if (!isRecord4(value) || !string(value.attemptId)) {
    throw new Error("\u5FAE\u4FE1\u626B\u7801\u670D\u52A1\u6CA1\u6709\u8FD4\u56DE\u6709\u6548\u7684\u7ED1\u5B9A\u4EFB\u52A1");
  }
  const status = PROVISION_STATES3.has(value.status) ? value.status : "failed";
  const result = {
    attemptId: string(value.attemptId),
    status,
    expiresAt: timestamp3(value.expiresAt) ?? Date.now(),
  ...
  const qrCodeDataUrl = safeQrSource4(value.qrCodeDataUrl);
...
function normalizeBot4(value) {
  if (!isRecord4(value) || !string(value.botId) || !isRecord4(value.bot)) return null;
  const state = ACCOUNT_STATES3.has(value.state) ? value.state : "error";
  const connected = value.connected === true;
  return {
...
function normalizeSnapshot3(value) {
  if (!isRecord4(value) || !Array.isArray(value.bots)) {
    throw new Error("\u5FAE\u4FE1\u670D\u52A1\u6CA1\u6709\u8FD4\u56DE\u6709\u6548\u7684\u8D26\u53F7\u5217\u8868");
  }
  const bots = value.bots.map(normalizeBot4).filter(Boolean);
  return {
    ...
    provisioning: value.provisioning ? normalizeProvisioning4(value.provisioning) : null
  };
}
...
function LoadingView4() {
  return h5(
    ...
    h5("div", { className: "dxw-spinner dim-spinner" }),
    h5("span", null, "\u6B63\u5728\u8BFB\u53D6\u5FAE\u4FE1\u8FDE\u63A5\u72B6\u6001\u2026")
  );
}
function EmptyView4({ onStart, busy }) {
  return h5(
    "div",
    ...
    h5(
      ...
      h5(
        ...
        h5(
          ...
          h5("span", { className: "dxw-dot dim-stateDot" }),
          h5("span", null, "\u5C1A\u672A\u7ED1\u5B9A\u5FAE\u4FE1")
        ),
        h5("h3", null, "\u626B\u4E00\u6B21\u7801\uFF0C\u5C31\u80FD\u5728\u5FAE\u4FE1\u91CC\u4F7F\u7528 Harness"),
        h5("p", null, "\u4E8C\u7EF4\u7801\u7531\u817E\u8BAF\u5FAE\u4FE1 iLink \u670D\u52A1\u7B7E\u53D1\u3002\u7528\u624B\u673A\u5FAE\u4FE1\u626B\u63CF\u5E76\u786E\u8BA4\u540E\uFF0C\u8D26\u53F7\u51ED\u636E\u4F1A\u76F4\u63A5\u5199\u5165 Harness Host\uFF0C\u6D4F\u89C8\u5668\u4E0D\u4F1A\u6536\u5230 bot_token\u3002"),
        h5(
          "div",
      ...
      h5("div", { className: "dxw-logo dim-emptyBrand", "aria-hidden": "true" }, h5(WeixinLogoGlyph, { size: 64 }))
    )
  );
}
    ...
    h5(
      ...
      h5(
        ...
        h5(
          ...
          source && !imageFailed ? h5("img", {
            src: source,
            alt: "\u7528\u4E8E\u628A\u5FAE\u4FE1\u673A\u5668\u4EBA\u7ED1\u5B9A\u5230 DeepSeek Harness \u7684\u4E00\u6B21\u6027\u4E8C\u7EF4\u7801",
            onError: () => setImageFailed(true)
          }) : h5("div", { className: "dxw-qrFallback dim-qrFallback" }, "\u4E8C\u7EF4\u7801\u56FE\u7247\u672A\u5C31\u7EEA\uFF0C\u8BF7\u4F7F\u7528\u5907\u7528\u94FE\u63A5\u3002"),
          expired ? h5("div", { className: "dxw-expired dim-qrExpired" }, "\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F\n\u8BF7\u91CD\u65B0\u751F\u6210") : null
        ),
        h5(
          "div",
          { className: "dxw-countdown dim-countdown" },
          h5("div", { className: "dim-countdownTop" }, h5("span", null, "\u4E8C\u7EF4\u7801\u6709\u6548\u65F6\u95F4"), h5("strong", null, formatRemaining4(remaining))),
          h5(
            "div",
            { className: "dxw-progress dim-progress", "aria-hidden": "true" },
            h5("span", { style: { "--dxw-progress": `${progress}%` } })
          )
        )
      ),
      h5(
        ...
        h5(
          "div",
          { className: "dxw-stateLabel dim-stateLabel" },
          h5("span", { className: "dxw-dot dim-stateDot", "data-tone": provision.status === "scanned" ? "success" : "warning" }),
          h5("span", null, provision.status === "scanned" ? "\u5DF2\u626B\u7801\uFF0C\u8BF7\u5728\u624B\u673A\u4E0A\u786E\u8BA4" : "\u7B49\u5F85\u5FAE\u4FE1\u626B\u7801")
        ),
        h5("h3", null, expired ? "\u4E8C\u7EF4\u7801\u5DF2\u5931\u6548" : "\u4F7F\u7528\u624B\u673A\u5FAE\u4FE1\u626B\u63CF\u4E8C\u7EF4\u7801"),
        h5("p", null, "\u8BF7\u5728\u624B\u673A\u4E0A\u6838\u5BF9\u5E76\u786E\u8BA4\u6388\u6743\u3002\u90E8\u5206\u8D26\u53F7\u4F1A\u989D\u5916\u663E\u793A\u4E00\u4E2A\u914D\u5BF9\u6570\u5B57\uFF0C\u9875\u9762\u4F1A\u5728\u9700\u8981\u65F6\u63D0\u793A\u8F93\u5165\u3002"),
        h5(
          "ol",
          { className: "dxw-steps dim-steps" },
          h5("li", null, "\u6253\u5F00\u624B\u673A\u5FAE\u4FE1\u5E76\u626B\u63CF\u5DE6\u4FA7\u4E8C\u7EF4\u7801"),
          h5("li", null, "\u5728\u5FAE\u4FE1\u4E2D\u786E\u8BA4\u8FDE\u63A5\u8BE5\u673A\u5668\u4EBA"),
          h5("li", null, "\u4FDD\u6301\u672C\u9875\u6253\u5F00\uFF0C\u7B49\u5F85\u6D88\u606F\u957F\u8F6E\u8BE2\u53D8\u4E3A\u5728\u7EBF")
        ),
        h5(
          "div",
          { className: "dxw-actions dim-viewActions" },
          expired ? h5(Button7, { kind: "primary", onClick: onRefresh, disabled: busy }, "\u91CD\u65B0\u751F\u6210\u4E8C\u7EF4\u7801") : null,
          href ? h5("a", {
          ...
          !expired ? h5(Button7, { onClick: onRefresh, disabled: busy }, "\u6362\u4E00\u4E2A\u4E8C\u7EF4\u7801") : null,
          h5(Button7, { onClick: onCancel, disabled: busy }, "\u53D6\u6D88")
        )
      )
    )
  );
...
function VerificationPanel({ provision, busy, onSubmit, onCancel }) {
  ...
  React6.useEffect(() => setCode(""), [provision.attemptId]);
  return h5(
    "div",
    { className: "dxw-card dim-surfaceCard" },
    h5(
      ...
      h5(
        ...
        h5("span", { className: "dxw-dot", "data-tone": "warning" }),
        h5("span", null, "\u9700\u8981\u914D\u5BF9\u7801")
      ),
      h5("h3", null, "\u8F93\u5165\u624B\u673A\u5FAE\u4FE1\u663E\u793A\u7684\u6570\u5B57"),
      h5("p", null, "\u8FD9\u662F\u5FAE\u4FE1\u9644\u52A0\u7684\u5B89\u5168\u786E\u8BA4\u6B65\u9AA4\u3002\u914D\u5BF9\u7801\u53EA\u7528\u4E8E\u672C\u6B21\u626B\u7801\u8F6E\u8BE2\uFF0C\u4E0D\u4F1A\u5199\u5165\u914D\u7F6E\u6216\u65E5\u5FD7\u3002"),
      h5(
        "div",
      ...
      h5(Button7, { onClick: onCancel, disabled: busy }, "\u53D6\u6D88\u7ED1\u5B9A")
    )
  );
}
function ProgressPanel2({ scanned, onCancel, busy }) {
  return h5(
    ...
    { className: "dxw-card dxw-loading dim-surfaceCard dim-loadingView", "aria-busy": "true" },
    h5("div", { className: "dxw-spinner dim-spinner" }),
    h5("h3", null, scanned ? "\u5FAE\u4FE1\u5DF2\u786E\u8BA4\uFF0C\u6B63\u5728\u542F\u52A8\u6D88\u606F\u8FDE\u63A5" : "\u6B63\u5728\u51C6\u5907\u5FAE\u4FE1\u4E8C\u7EF4\u7801"),
    h5("p", null, scanned ? "\u6B63\u5728\u4FDD\u5B58\u51ED\u636E\u5E76\u9A8C\u8BC1 Harness \u4E0E\u5FAE\u4FE1\u957F\u8F6E\u8BE2\u3002" : "\u6B63\u5728\u8054\u7CFB\u817E\u8BAF\u5FAE\u4FE1 iLink \u670D\u52A1\u3002"),
    onCancel ? h5(
      "div",
...
function ProvisionError3({ provision, busy, onRetry, onClose }) {
  ...
  return h5(
    ...
    h5(
      ...
      h5("h3", null, provision.status === "expired" ? "\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F" : "\u5FAE\u4FE1\u6CA1\u6709\u7ED1\u5B9A\u5B8C\u6210"),
      h5("p", null, error.message),
      h5("span", { className: "dxw-errorCode" }, error.code),
      h5(
        "div",
        { className: "dxw-actions dim-viewActions" },
...
function checkedTime3(timestamp4) {
  if (!timestamp4) return "\u5C1A\u672A\u68C0\u67E5";
  try {
    return new Intl.DateTimeFormat("zh-CN", {
      hour: "2-digit",
...
function AccountCard3({ account, busy, removing, onReconnect, onRequestRemove, onConfirmRemove, onCancelRemove }) {
  ...
  return h5(
    ...
    h5(
      ...
      h5(
        ...
        h5(
          ...
          h5("div", { className: "dxw-avatar dim-botAvatar", "aria-hidden": "true" }, h5(WeixinLogoGlyph, { size: 27 })),
          h5("div", { className: "dim-botName" }, h5("h3", null, account.bot.name), h5("p", null, account.bot.accountIdMasked))
        ),
        h5(
          "div",
          { className: "dxw-health dim-botHealth" },
          h5("span", { className: "dxw-dot dim-healthDot", "data-tone": tone }),
          h5("span", null, account.connected ? "\u8FD0\u884C\u6B63\u5E38" : state === "connecting" ? "\u6B63\u5728\u8FDE\u63A5" : "\u8FDE\u63A5\u672A\u5C31\u7EEA")
        )
      ),
      h5(
        "dl",
        ...
        h5(
          ...
          h5("dt", null, "\u6D88\u606F\u901A\u9053"),
          h5("dd", null, account.connected ? "iLink \u957F\u8F6E\u8BE2" : "\u79BB\u7EBF")
        ),
        h5(
          "div",
          { className: "dxw-metric dim-botMetric" },
          h5("dt", null, "\u6700\u8FD1\u68C0\u67E5"),
          h5("dd", null, checkedTime3(account.health.lastCheckedAt))
        )
      ),
      h5(
        "div",
        { className: "dxw-accountFooter dim-cardFooter" },
        h5("div", { className: "dxw-summary dim-cardSummary" }, account.error?.message ?? account.health.summary),
        h5(
          "div",
    ...
    removing ? h5(
      ...
      h5("strong", null, "\u4ECE\u6B64 Harness \u79FB\u9664\u8FD9\u4E2A\u5FAE\u4FE1\u8D26\u53F7\uFF1F"),
      h5("p", null, "\u8FD9\u4F1A\u505C\u6B62\u6D88\u606F\u8FDE\u63A5\uFF0C\u5E76\u5220\u9664\u672C\u673A\u4FDD\u5B58\u7684 bot_token\u3001\u8D26\u53F7\u914D\u7F6E\u548C\u4F1A\u8BDD\u6620\u5C04\u3002\u5176\u4ED6\u5FAE\u4FE1\u8D26\u53F7\u4E0D\u53D7\u5F71\u54CD\u3002"),
      h5(
        "div",
        { className: "dxw-actions dim-viewActions" },
        h5(Button7, { onClick: onCancelRemove, disabled: busy === "delete" }, "\u4FDD\u7559\u8D26\u53F7"),
        h5(
          Button7,
...
function AccountList2(props) {
  return h5(
    "section",
    { className: "dim-listSection" },
    h5("div", { className: "dxw-listHeading dim-listHeading" }, h5("h3", null, "\u5DF2\u63A5\u5165\u7684\u5FAE\u4FE1\u8D26\u53F7"), h5("span", null, `${props.bots.length} \u4E2A`)),
    h5("ul", { className: "dxw-list dim-botList" }, props.bots.map((account) => h5(
      "li",
      { key: account.botId },
      h5(AccountCard3, {
        account,
...
function WeixinSettingsTab({ rpcCall }) {
  ...
  const scheduleAnimationFrame = useAnimationFrameScheduler();
  const announce = React6.useCallback((value) => {
    setNotice("");
    scheduleAnimationFrame(() => {
  ...
  const loadStatus = React6.useCallback(async ({
    ...
    try {
      const snapshot = normalizeSnapshot3(await invoke(WEIXIN_ENDPOINTS.status, {}, signal));
      if (signal?.aborted) return void 0;
      setModel({
        phase: "ready",
        bots: snapshot.bots,
  ...
  const startProvisioning = React6.useCallback(async ({ replace = false } = {}) => {
    ...
    try {
      ...
      setProvision({ status: "starting" });
      const started = normalizeProvisioning4(await invoke(WEIXIN_ENDPOINTS.beginProvisioning, { locale: "zh-CN" }));
      setNow(Date.now());
      setProvision({ ...started, durationMs: Math.max(1, started.expiresAt - Date.now()) });
  ...
  const submitVerification = React6.useCallback(async (verifyCode) => {
    if (!provision?.attemptId) return;
    setBusy(true);
    try {
      const next = normalizeProvisioning4(await invoke(WEIXIN_ENDPOINTS.submitVerification, {
        attemptId: provision.attemptId,
        verifyCode
      }));
  ...
  React6.useEffect(() => {
    ...
    const poll = async () => {
      try {
        const result = normalizeProvisioning4(await invoke(
        ...
        setProvision((current) => current?.attemptId === attemptId ? { ...current, status: "failed", error: presentError4(error) } : current);
      }
    };
  ...
  const reconnect = React6.useCallback(async (account) => {
    setBotBusy(account.botId, "reconnect");
    try {
      const snapshot = normalizeSnapshot3(await invoke(WEIXIN_ENDPOINTS.reconnectBot, { botId: account.botId }));
      ...
      announce(`\u8FDE\u63A5\u68C0\u67E5\u5931\u8D25\uFF1A${presentError4(error).message}`);
    } finally {
  ...
  const remove = React6.useCallback(async (account) => {
    ...
    try {
      const snapshot = normalizeSnapshot3(await invoke(WEIXIN_ENDPOINTS.deleteBot, {
        botId: account.botId,
        confirm: true
      }));
      setModel((current) => ({ ...current, bots: snapshot.bots, totals: snapshot.totals, revision: snapshot.revision }));
      ...
      announce(`\u79FB\u9664\u5931\u8D25\uFF1A${presentError4(error).message}`);
    } finally {
      setBotBusy(account.botId, null);
    }
  }, [announce, invoke, setBotBusy]);
  let provisionView = null;
  if (provision?.status === "starting") {
    provisionView = h5(ProgressPanel2, { busy });
  } else if (["pending", "scanned"].includes(provision?.status)) {
    provisionView = h5(QrPanel3, {
      provision,
      now,
      busy,
      onRefresh: () => void startProvisioning({ replace: true }),
    ...
    provisionView = h5(VerificationPanel, {
      provision,
      busy,
    ...
    provisionView = h5(ProgressPanel2, {
      scanned: true,
      busy,
      onCancel: () => void cancelProvisioning()
    });
  } else if (provision && ["failed", "expired", "cancelled"].includes(provision.status)) {
    provisionView = h5(ProvisionError3, {
      provision,
      busy,
      onRetry: () => void startProvisioning({ replace: Boolean(provision.attemptId) }),
      onClose: () => void cancelProvisioning()
  ...
  return h5(
    ...
    h5("div", { className: "dxw-visuallyHidden", role: "status", "aria-live": "polite" }, notice),
    model.error && model.phase === "ready" ? h5("div", { className: "dxw-statusNotice dim-statusNotice" }, `\u72B6\u6001\u5237\u65B0\u5931\u8D25\uFF1A${model.error.message}`) : null,
    model.phase === "loading" ? h5(LoadingView4) : model.phase === "error" ? h5(
      "div",
      { className: "dxw-card dim-surfaceCard" },
      h5(
        "div",
        ...
        h5("h3", null, "\u65E0\u6CD5\u8BFB\u53D6\u5FAE\u4FE1\u72B6\u6001"),
        h5("p", null, model.error?.message ?? "\u8BF7\u7A0D\u540E\u91CD\u8BD5"),
        h5(Button7, { onClick: () => void loadStatus() }, "\u91CD\u65B0\u8BFB\u53D6")
      )
    ) : h5(
      React6.Fragment,
      null,
      provisionView,
      model.bots.length === 0 && !provision ? h5(EmptyView4, { onStart: () => void startProvisioning(), busy }) : null,
      model.bots.length > 0 ? h5(AccountList2, {
        bots: model.bots,
        busyByBot,
        removeTarget,
        onReconnect: (account) => void reconnect(account),
...
function DingtalkLogo() {
  return h6(
    ...
    h6(DingtalkLogoGlyph)
  );
}
function QqLogo() {
  return h6("span", { className: "dim-logo dim-logoQq", "aria-hidden": "true" }, h6(QqLogoGlyph));
}
function ChannelLogo({ channel }) {
  if (channel === "weixin") return h6(WeixinLogo);
  if (channel === "feishu") return h6(FeishuLogo);
  if (channel === "dingtalk") return h6(DingtalkLogo);
  return h6(QqLogo);
}
function IMSettingsTab({ dingtalkRpcCall, feishuRpcCall, qqRpcCall, weixinRpcCall }) {
  const [selected, setSelected] = React7.useState("weixin");
  ...
  return h6(
    ...
    h6(
      "header",
      { className: "dim-title" },
      h6("p", null, "\u901A\u8FC7\u626B\u7801\u628A\u673A\u5668\u4EBA\u63A5\u5165 DeepSeek Harness")
    ),
    h6(
      "div",
      ...
      h6(
        ...
        CHANNELS.map((channel) => h6(
          ...
          h6(ChannelLogo, { channel: channel.id }),
          h6(
            "span",
            { className: "dim-channelCopy" },
            h6("strong", null, channel.label)
          )
        ))
      ),
      h6("div", { className: "dim-divider", "aria-hidden": "true" }),
      h6("main", {
        className: "dim-panel",
        role: "tabpanel",
        id: `dim-panel-${active.id}`,
        "aria-labelledby": `dim-tab-${active.id}`
      }, active.id === "weixin" ? h6(WeixinSettingsTab, { rpcCall: weixinRpcCall }) : active.id === "feishu" ? h6(FeishuSettingsTab, { rpcCall: feishuRpcCall }) : active.id === "dingtalk" ? h6(DingtalkSettingsTab, { rpcCall: dingtalkRpcCall }) : h6(QqSettingsTab, { rpcCall: qqRpcCall }))
    )
  );
}
function apply(ctx) {
