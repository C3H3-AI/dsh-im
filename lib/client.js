...
function useAnimationFrameScheduler() {
  const schedulerRef = React2.useRef(null);
  React2.useEffect(() => {
...
var Button3 = React3.forwardRef(function Button4({ children, kind = "secondary", size, icon, className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function QrPane({ provision, now, onRefresh, onCancel, busy }) {
  ...
  React3.useEffect(() => setImageFailed(false), [qrSource]);
  return h2(
    "div",
...
function FeishuSettingsTab({ rpcCall }) {
  ...
  const [pageBusy, setPageBusy] = React3.useState(false);
  const [provisionBusy, setProvisionBusy] = React3.useState(false);
  const [busyByBot, setBusyByBot] = React3.useState({});
  const [errorsByBot, setErrorsByBot] = React3.useState({});
  const [removeTargetId, setRemoveTargetId] = React3.useState(null);
  const [announcement, setAnnouncement] = React3.useState("");
  const [now, setNow] = React3.useState(() => Date.now());
  const [focusBotId, setFocusBotId] = React3.useState(null);
  const cardRefs = React3.useRef(/* @__PURE__ */ new Map());
  const removeButtonRefs = React3.useRef(/* @__PURE__ */ new Map());
  ...
  const mergeSnapshot = React3.useCallback((snapshot, { restoreProvisioning = false } = {}) => {
    const now2 = Date.now();
  ...
  const loadStatus = React3.useCallback(async ({ signal, silent = false, restoreProvisioning = false } = {}) => {
  ...
  const setBotError = React3.useCallback((botId, error) => {
  ...
  const reconnectOneBot = React3.useCallback(async (connection) => {
  ...
  const requestRemove = React3.useCallback((connection) => {
    setRemoveTargetId(connection.botId);
  }, []);
  const cancelRemove = React3.useCallback(() => {
  ...
  const confirmRemove = React3.useCallback(async (connection) => {
    const { botId, bot } = connection;
  ...
  const setCardRef = React3.useCallback((botId, node) => {
    if (node) cardRefs.current.set(botId, node);
    else cardRefs.current.delete(botId);
  }, []);
  const setRemoveButtonRef = React3.useCallback((botId, node) => {
...
var Button5 = React4.forwardRef(function Button6({ children, kind = "secondary", className = "", ...props }, ref) {
  return h3("button", {
    ...props,
    ref,
    type: "button",
...
function VerificationPanel({ provision, busy, onSubmit, onCancel }) {
  ...
  React4.useEffect(() => setCode(""), [provision.attemptId]);
  return h3(
    "div",
    { className: "dxw-card" },
    h3(
...
function WeixinSettingsTab({ rpcCall }) {
  ...
  const scheduleAnimationFrame = useAnimationFrameScheduler();
  const announce = React4.useCallback((value) => {
    setNotice("");
    scheduleAnimationFrame(() => {
  ...
  const submitVerification = React4.useCallback(async (verifyCode) => {
  ...
  const reconnect = React4.useCallback(async (account) => {
...
function IMSettingsTab({ dingtalkRpcCall, feishuRpcCall, weixinRpcCall }) {
  const [selected, setSelected] = React5.useState("weixin");
