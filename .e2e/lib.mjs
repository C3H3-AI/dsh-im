...
export const AGENT = 'c3h3dsh@agent.qq.com';
...
export async function send({ to, subject, text, attachments, headers }) {
  const info = await transport().sendMail({ from: QQ, to, subject,
    ...(text !== undefined ? { text } : {}),
...
export function readState(botId) {
  return JSON.parse(readFileSync(
    `/home/duola/.dsh/integrations/dsh-email/bots/${botId}/state.json`,'utf8'));
}
